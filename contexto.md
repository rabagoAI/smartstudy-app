...
| 2026-06-16 | SEC-12 | Auditoría de privilegios con `scripts/auditar_admin.py` (Admin SDK, solo lectura): **sin explotación** — solo el admin legítimo, claim Auth sincronizado, ningún premium sin rastro de Stripe | Claude Code |
| 2026-06-16 | UX | Vídeos de Asignaturas migrados de `<iframe>` a `<video controls>` nativo (`SubjectDetailsPage.jsx`, componente `VideoPlayer`); mejor UX y no depende de `frame-src`; mantiene iframe solo para YouTube | Claude Code |
| 2026-06-16 | #13 | Tests del webhook Stripe (`api/_lib/stripeWebhook.ts` extraído + 10 tests) y de las reglas Firestore (25 tests con emulador, `tests/firestore.rules.test.ts`); script `test:rules`, job CI con Java, `firebase-tools@13` devDep | Claude Code |
...
- [x] **#13 — Tests + CI** (2026-06-15): runner Vitest configurado en `vite.config.js` (jsdom, setupTests), scripts `test`/`test:watch`, workflow `.github/workflows/ci.yml` (install→typecheck→test→build). Lógica de truncado de chat extraída a `src/lib/chatHistory.ts` con tests; tests de `cn`. **9 tests verdes.**
  - **(2026-06-16) Tests webhook Stripe + reglas Firestore añadidos.** Lógica del webhook extraída a `api/_lib/stripeWebhook.ts` (`handleStripeEvent`) con 10 tests (Firestore/Stripe mockeados). Reglas Firestore: 25 tests con `@firebase/rules-unit-testing` contra el emulador (`tests/firestore.rules.test.ts`, `npm run test:rules`). Job CI `rules` separado con Java 17. `firebase-tools@13` devDep (el v15 global exige Java 21). Total: 19 tests unit + 25 reglas.
